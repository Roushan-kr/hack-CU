...
function useBusStopsForBothDirections(
  ...
  const [forwardStops, setForwardStops] = useState<BusStopResponse | null>(
    null
  );
  const [backwardStops, setBackwardStops] = useState<BusStopResponse | null>(
    null
  );
  ...
  return (
    ...
    >
      <Routes>
        ...
        <Route
          path="/lines/:lineNumber/directions"
          element={<DirectionsPage />}
        />
        <Route
          path="/lines/:lineNumber/directions/:direction/stops"
          element={<DirectionStopsPage />}
        />
        <Route
          path="/lines/:lineNumber/directions/:direction/tracking"
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-primary-100 text-sm">
                  Choose your bus line to get started
                </p>
              </div>
            </div>
...
function DirectionsPage() {
  ...
  const handleDirectionSelect = (
    ...
    navigate(
      `/lines/${lineNumber}/directions/${direction.toLowerCase()}/stops`
    );
  };
...
function DirectionStopsPage() {
  const { lineNumber, direction } = useParams<{
    lineNumber: string;
    direction: string;
  }>();
  const navigate = useNavigate();
          ...
          <div className="flex items-center justify-between max-w-6xl mx-auto">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-primary-100 text-sm">
                  View all bus stops ({stops.length} stops)
                </p>
              </div>
            </div>
      ...
      <div className="pt-24">
        <AnimatePresence mode="wait">
          ...
          >
            <DirectionSelector
              forwardLine={
                direction?.toUpperCase() === 'FORWARD' ? selectedLine : null
              }
              backwardLine={
                direction?.toUpperCase() === 'BACKWARD' ? selectedLine : null
              }
              selectedDirection={
                direction?.toUpperCase() as 'FORWARD' | 'BACKWARD'
              }
              onDirectionSelect={handleStopSelect}
              forwardStops={direction?.toUpperCase() === 'FORWARD' ? stops : []}
              backwardStops={
                direction?.toUpperCase() === 'BACKWARD' ? stops : []
              }
              userLocation={userLocation}
              showStopsView={true}
              onBack={handleBack}
...
function TrackingPage() {
  ...
  const { data: busStopsData, loading: currentStopsLoading } = useBusStops(
